...
const More = () => {
  ...
  const [showNewAccountInput, setShowNewAccountInput] = useState(''); // '', 'client_payment', 'vendor_payment', 'vendor_expense'
    ...
    try {
      ...
      if (data && data[0]) {
        ...
        if (targetForm === 'client_payment') {
          setPaymentForm(prev => ({ ...prev, account: newAcc.name }));
        } else if (targetForm === 'vendor_payment') {
          setPaymentForm(prev => ({ ...prev, account_id: newAcc.id }));
        } else if (targetForm === 'vendor_expense') {
          setVendorExpenseForm(prev => ({ ...prev, account_id: newAcc.id }));
        }
                  ...
                  <div className="flex items-center gap-4 bg-slate-50 border p-3 rounded-xl">
                    <div className="text-center">
                      ...
                      <span className="text-md font-bold text-red-600">
                        {totalBal >= 0 ? `₹${totalBal.toLocaleString('en-IN')}` : `-₹${Math.abs(totalBal).toLocaleString('en-IN')}`}
                      </span>
                    </div>
                    <button 
                      onClick={() => {
                    ...
                    <select 
                      ...
                      onChange={e => {
                        if (e.target.value === 'ADD_NEW') {
                          ...
                          setPaymentForm({...paymentForm, account: ''});
                        } else {
                    ...
                    <select 
                      ...
                      onChange={e => {
                        if (e.target.value === 'ADD_NEW') {
                          ...
                          setPaymentForm({...paymentForm, account_id: ''});
                        } else {
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <select 
                    ...
                    onChange={e => {
                      if (e.target.value === 'ADD_NEW') {
                        setShowNewAccountInput('vendor_expense');
                        setVendorExpenseForm({...vendorExpenseForm, account_id: ''});
                      } else {
                        setVendorExpenseForm({...vendorExpenseForm, account_id: e.target.value || ''});
                      }
                    }}
                  >
                    <option value="">-- None (Unpaid Invoice) --</option>
                  ...
                  {showNewAccountInput === 'vendor_expense' && (
                    <div className="mt-2 flex items-center space-x-2">
                      <input 
                        type="text" 
                        className="input-field py-1 text-xs flex-1" 
                      ...
                      <button 
                        type="button" 
                        onClick={() => handleAddNewAccount('vendor_expense')}
                        className="bg-primary-600 hover:bg-primary-700 text-white font-semibold px-2 py-1 rounded text-xs cursor-pointer"
                      >
                        Add
                      </button>
